...
extern const NSTimeInterval kSKAnimatedImageDelayTimeIntervalMinimum;

//
//  An `SKAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `SKAnimatedImageView`.
...
//
@interface SKAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `SKAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `SKAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
typedef NS_ENUM(NSUInteger, SKLogLevel) {
    SKLogLevelNone = 0,
    SKLogLevelError,
    SKLogLevelWarn,
    SKLogLevelInfo,
...
@interface SKAnimatedImage (Logging)
...
#define SKLog(logLevel, format, ...) [SKAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface SKWeakProxy : NSProxy
